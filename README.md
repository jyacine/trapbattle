...
The default lobby host is `34-155-132-207.nip.io` (the live GCP VM). Change it in `scripts/lobby_ui.gd` or type a different address in the lobby UI.

---

## Networking

- Connects via `wss://34-155-132-207.nip.io` (port 443, TLS terminated by Caddy on the VM).
...
| Server | Live VM at `34.155.132.207` must be running; **no other players connected** |
...
Both headless processes connect via `wss://34-155-132-207.nip.io` with default TLS
